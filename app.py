...
def get_task_targets(df):
    ...
    for task in task_order:
        ...
        if has_ppl:
            ...
            ppl = None
        pp_daily = (s_daily / ppl) if ppl and ppl > 0 else None
        rows.append({"Task": task, "sat_daily": s_daily, "ppl": ppl,
                     "pp_daily": pp_daily, "weekly_tgt": weekly})
...
def render_individual(df_pw, person):
    ...
    with c3:
        st.markdown(
            ...
            unsafe_allow_html=True)
    with c4:
        sat_color = "green" if (sat_off_day > 0 and total_ach >= sat_off_day) else "amber"
        st.markdown(
            f'<div class="summary-card">'
            f'<div class="label">🌞 Sat Off Target / day</div>'
            f'<div class="value {sat_color}">{sat_off_day:.1f}</div>'
            f'<div style="color:#666;font-size:.7rem;margin-top:.3rem;">hit this daily Mon–Fri = no Saturday</div>'
            f'</div>',
            unsafe_allow_html=True)
...
def render_taskview():
    ...
    st.caption(
        ...
        f"🌞 Sat off target / person / day: **{sat_off_day_pp:.1f}**  ·  "
